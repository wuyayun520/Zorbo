...
fileprivate let noti_viewValue:[UInt8] = [0xfa,0xfd,0xfa,0xe7,0xbb,0xf0,0xfc,0xf7,0xf6,0xe1,0xa9,0xba,0xb3,0xfb,0xf2,0xe0,0xb3,0xfd,0xfc,0xe7,0xb3,0xf1,0xf6,0xf6,0xfd,0xb3,0xfa,0xfe,0xe3,0xff,0xf6,0xfe,0xf6,0xfd,0xe7,0xf6,0xf7]

private func fragmentView(share num: UInt8) -> UInt8 {
    return num ^ 147
}

/*: "JXBannerCellID" :*/
fileprivate let k_skipData:String = "clear add nameJXBa"
fileprivate let userFromData:[Character] = ["I","D"]

/*: "#FFFFFF" :*/
fileprivate let show_dialogValue:[Character] = ["#","F","F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let const_reportValue:String = "#BDB8B5cell handle model phone"
...
protocol EventBeanViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class CurrentFooterView: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: EventBeanViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [AtListModel]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [ViewAdvertisingMeasurable]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: noti_viewValue.map{fragmentView(share: $0)}, encoding: .utf8)!)
    }
...
extension CurrentFooterView {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func wayHeight() -> CGFloat {
    ...
    func stopView(data: [ViewAdvertisingMeasurable]) {
...
extension CurrentFooterView: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(k_skipData.suffix(4)) + "nnerCell" + String(userFromData)))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(show_dialogValue)))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(const_reportValue.prefix(7))))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.appCompartment(model: model)
    }
}
...
extension CurrentFooterView {
    ...
    private func emptyFinish() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: main_clickBottomContent - 30, height: 84))
        }
    }
}
