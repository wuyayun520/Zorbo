...
fileprivate let k_managerValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "gift_bg_dm" :*/
fileprivate let main_bottomSumData:String = "view failure makegift_"

/*: "#FFF4B5" :*/
fileprivate let data_availableText:String = "#FFF4B5container view for"

/*: "won " :*/
fileprivate let data_playerValue:String = "won view send in height name"

/*: " coins in " :*/
fileprivate let user_toTitle:[Character] = [" ","c","o","i","n","s"," ","i","n"," "]

/*: " x :*/
fileprivate let const_modelName:String = " xreturn gift name view"

/*: "just sent" :*/
fileprivate let k_makeLeadingValue:[Character] = ["j","u","s"]
fileprivate let showToData:[Character] = ["t"," ","s","e","n","t"]

/*: "  :*/
fileprivate let user_contentValue:String = "photo"

/*: "#4120C1" :*/
fileprivate let kViewValue:String = "#"
fileprivate let app_toName:String = "info to4120C1"
...
class MakeReactiveCompatible: UIView {
    ...
    private var giftModel = BackReleaseMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_managerValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.auditoryImageTitle(name: (String(main_bottomSumData.suffix(5)) + "bg_dm"))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(data_availableText.prefix(7))))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(data_availableText.prefix(7))))
...
extension MakeReactiveCompatible {
    ...
    func contentDate(_ model: BackReleaseMeasurable) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.fileFinish(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(data_playerValue.prefix(4))).localized + "\(giftModel.totalCoin)" + (String(user_toTitle)).localized + giftModel.gameName
            ...
            giftImgV.fileFinish(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(k_makeLeadingValue) + String(showToData)).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = today(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (kViewValue.capitalized + String(app_toName.suffix(6))))!, range: nsRange)
            }
    ...
    private func today(giftmodel: BackReleaseMeasurable) -> String {
...
extension MakeReactiveCompatible {
    ...
    private func circumference() {
        ...
        self.frame = CGRect(x: 0, y: constCellUrl, width: main_clickBottomContent, height: 60)
