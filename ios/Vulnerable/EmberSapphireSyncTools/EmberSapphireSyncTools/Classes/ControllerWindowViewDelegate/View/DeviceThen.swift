...
fileprivate let kUpTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let appUserData:[Character] = ["m","o","v","e"]
...
class DeviceThen: UIView {
	var systematicallyName: String?
	var pianoArray: [AnyHashable]?
    ...
    var danumuMsgArray: [CapabilityModelType] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [RequestView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<CapabilityModelType?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kUpTitle.reversed(), encoding: .utf8)!)
    }
    ...
    deinit {
        ...
        removeGestureRecognizer(tapGesture)
    		systematicallyName = nil
		pianoArray = nil

	}

    //: func didReceive(_ danumuMsgModel: TalkingFloatingScreenModel?) {
    func perspective(_ danumuMsgModel: CapabilityModelType?) {
    ...
    @objc func pastControl(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(pastControl(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension DeviceThen: UIGestureRecognizerDelegate {
    ...
    func upColor(for danumuMsgModel: CapabilityModelType?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = RequestView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(appUserData)))
    ...
    func infoChannel(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                upColor(for: model, isFirst: isFirst)
