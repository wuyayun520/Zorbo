...
/*: s" :*/
fileprivate let kTopAppTitle:String = "to"

/*: "init(coder:) has not been implemented" :*/
fileprivate let dataPhotoManagerName:[UInt8] = [0x7e,0x79,0x7e,0x63,0x3f,0x74,0x78,0x73,0x72,0x65,0x2d,0x3e,0x37,0x7f,0x76,0x64,0x37,0x79,0x78,0x63,0x37,0x75,0x72,0x72,0x79,0x37,0x7e,0x7a,0x67,0x7b,0x72,0x7a,0x72,0x79,0x63,0x72,0x73]

private func queryFrame(time num: UInt8) -> UInt8 {
    return num ^ 23
}

/*: "bth_live_follow_pre" :*/
fileprivate let userMakeValue:[Character] = ["b","t","h","_","l","i","v","e","_","f","o"]
fileprivate let dataArrayText:String = "llolength"

/*: "btn_live_mini_nor" :*/
fileprivate let main_dataValue:String = "gift text object up elsebtn_live_"
fileprivate let const_fromTitle:String = "mini_nortable true as temp"

/*: "#2DF2FF" :*/
fileprivate let app_limitTitle:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let appTipPartyAttributeData:String = "table height self templateLive"

/*: "#FF506D" :*/
fileprivate let main_mData:String = "#FF506Dclear any to"

/*: "  :*/
fileprivate let noti_layerValue:String = " "

/*: "removeAttentionUid" :*/
fileprivate let app_shadowValue:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let mainPicNorNumbereractionName:[Character] = ["a","t","t","e","n"]
fileprivate let constTableToolText:String = "tionUidmake name equal view"

/*: "source" :*/
fileprivate let app_blockImageName:String = "valueource"

/*: "num" :*/
fileprivate let main_containerName:String = "nuvoice"

/*: "UID: :*/
fileprivate let main_makeData:String = "UID:self image self model"

/*: "btn_live_closed_nor" :*/
fileprivate let show_tableTitle:String = "self domainbtn_l"
fileprivate let userPlayerValue:[Character] = ["c","l","o","s","e","d","_","n","o","r"]
...
class AtTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: EqualViewController?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= SceneAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                self.countdownLab.text = "\(dataVersionBarSkipName - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.lightQuantitydown(countdown: "\(dataVersionBarSkipName - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = ExceptHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataPhotoManagerName.map{queryFrame(time: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(reportChange), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.fontJump(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.auditoryImageTitle(name: (String(userMakeValue) + dataArrayText.replacingOccurrences(of: "length", with: "w") + "_pre")), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(anObjectClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.auditoryImageTitle(name: (String(main_dataValue.suffix(9)) + String(const_fromTitle.prefix(8)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(toolPostClick), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: IndexStopView = {
        ...
        let image = UIImage.atomicQuantity85(color: UIColor(hex: (String(app_limitTitle)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circle(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(appTipPartyAttributeData.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(main_mData.prefix(7))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(SceneAppManager.share.appConfigMode.countdown2)s"
        lab.text = " \(SceneAppManager.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: ClickQuantitydownView = {
        //: let v = TalkingLiveCountdownView()
        let v = ClickQuantitydownView()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.clearTabPage(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(colorSnap), for: .touchUpInside)
    ...
    private lazy var onlineListView: OpticThen = {
...
extension AtTopView {
    ...
    @objc func modelSound(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(app_shadowValue))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(mainPicNorNumbereractionName) + String(constTableToolText.prefix(7)))] = uid
            //: dict["source"] = "1"
            dict[(app_blockImageName.replacingOccurrences(of: "value", with: "s"))] = "1"
        }
        //: LabelUpReactiveCompatible.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        LabelUpReactiveCompatible.dropEnable(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension AtTopView {
    ...
    @objc private func colorSnap() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.build(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func acquireFilter() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        GiftMotionPictureFanAudienceManager().sight(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(main_containerName.replacingOccurrences(of: "voice", with: "m"))].string
    ...
    private func twine() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(acquireFilter), userInfo: nil, repeats: true)
    }
...
extension AtTopView {
    ...
    func black(_ liveModel: ExceptHandyJSON) {
        ...
        uidLab.text = (String(main_makeData.prefix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.hideSizeFinish(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.sexThen(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.auditoryImageTitle(name: (String(main_dataValue.suffix(9)) + String(const_fromTitle.prefix(8)))), for: .normal)
            //: refreshFollow(isAttention: true)
            starAttention(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.auditoryImageTitle(name: (String(show_tableTitle.suffix(5)) + "ive_" + String(userPlayerValue))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            starAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }

    /// 最小化/关闭直播 按钮点击事件
    //: @objc func miniButtonClick() {
    @objc func toolPostClick() {
        //: rightBtnBlock?()
        rightBtnBlock?()
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == SceneAppManager.share.loginUid)
        ...
        if isAnchor { // 改为小窗口
            ...
            DataPushListener.handleCount().faceTop(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            occupier()?.navigationController?.popViewController(animated: false)
        }
    }

    //: @objc private func iconBgButtonClick() {
    @objc private func reportChange() {
        //: showUserCard(uid: String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid))
        exhibit(uid: String(DataPushListener.handleCount().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func exhibit(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = PlayerCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any RelinquishObjectProtocol
        //: userCard.show()
        userCard.shouldFromButton()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func starAttention(isAttention: Bool) {
    ...
    private func liveTogether() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !DataPushListener.handleCount().isLive
    ...
    @objc private func anObjectClick() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        modelSound(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.starAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension AtTopView {
    ...
    private func perimeter() {
        ...
        self.frame = CGRect(x: 0, y: 0, width: main_clickBottomContent, height: dataSaltName + actualWidth(w: 68))
