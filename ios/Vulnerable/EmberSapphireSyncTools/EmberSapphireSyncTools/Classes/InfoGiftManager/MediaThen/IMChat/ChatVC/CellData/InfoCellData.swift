...
/*: "title" :*/
fileprivate let main_tempTableCellValue:String = "textitextle"

/*: "content" :*/
fileprivate let notiImageName:String = "cotagtetagt"

/*: "init(coder:) has not been implemented" :*/
fileprivate let const_sizeText:[UInt8] = [0x25,0x22,0x25,0x38,0x64,0x2f,0x23,0x28,0x29,0x3e,0x76,0x65,0x6c,0x24,0x2d,0x3f,0x6c,0x22,0x23,0x38,0x6c,0x2e,0x29,0x29,0x22,0x6c,0x25,0x21,0x3c,0x20,0x29,0x21,0x29,0x22,0x38,0x29,0x28]

private func makeBottom(task num: UInt8) -> UInt8 {
    return num ^ 76
}

/*: "img" :*/
fileprivate let app_topValue:[Character] = ["i","m","g"]

/*: "jumpKey" :*/
fileprivate let userTargetTitle:[Character] = ["j","u","m","p","K","e"]
fileprivate let const_textValue:[Character] = ["y"]
...
@objcMembers public class InfoCellData: ChatCellData {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.collectionFor(), .font: UIFont.handleFont(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(main_tempTableCellValue.replacingOccurrences(of: "text", with: "t"))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.photoEvent(), .font: UIFont.handleFont(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(notiImageName.replacingOccurrences(of: "tag", with: "n"))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_sizeText.map{makeBottom(task: $0)}, encoding: .utf8)!)
    }
    ...
    override public func will(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(String(app_topValue))].stringValue.count > 0 {
        ...
        if extraJson[(main_tempTableCellValue.replacingOccurrences(of: "text", with: "t"))].stringValue.count > 0 {
        ...
        if extraJson[(notiImageName.replacingOccurrences(of: "tag", with: "n"))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(userTargetTitle) + String(const_textValue))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
