...
fileprivate let notiLiveContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_lounge" :*/
fileprivate let constMainName:String = "for path in model modelicon_l"
fileprivate let k_upValue:[Character] = ["o","u","n","g","e"]

/*: "icon_talk_points" :*/
fileprivate let userFromText:String = "icmaken"
fileprivate let showRowName:String = "_pointstring object"
fileprivate let const_cellName:String = "match"

/*: "C6BDFF" :*/
fileprivate let showImageColorData:[Character] = ["C","6","B","D","F","F"]

/*: "#BBBBBB" :*/
fileprivate let showEqualText:String = "data share#BBBBBB"

/*: "Reply to get points~" :*/
fileprivate let dataRangeContent:[Character] = ["R","e","p","l","y"," ","t"]
fileprivate let noti_toName:[Character] = ["o"," ","g","e"]
fileprivate let data_modeValue:String = "array hide if request viewt poi"

/*: "bth_unread_nor" :*/
fileprivate let userTextAtViewTitle:[Character] = ["b","t","h","_","u","n","r","e","a"]
fileprivate let appBottomText:[Character] = ["d","_","n","o","r"]

/*: "icon_male_default" :*/
fileprivate let app_calendarValue:[UInt8] = [0xa0,0xaa,0xa6,0xa7,0x96,0xa4,0xa8,0xa5,0xac,0x96,0xad,0xac,0xaf,0xa8,0xbc,0xa5,0xbd]

/*: "icon_female_default" :*/
fileprivate let notiSizeData:String = "icon_gesture tab view request keep"
fileprivate let showSystemResultName:String = "photo component rake userle_def"

/*: "   " :*/
fileprivate let k_checkStateTitle:String = "videovideovideo"

/*: "+%@ points" :*/
fileprivate let dataErrorDayElseName:String = "collection%@ "
fileprivate let user_partyData:[Character] = ["p","o","i","n","t","s"]

/*: "bth_read_pre" :*/
fileprivate let constViewPriceStatusTitle:String = "bth_make type view voice"
...
//: class TalkingChatBaseMsgCell: ImageProductMessageCell {
class AppMsgCell: ImageProductMessageCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiLiveContent.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.auditoryImageTitle(name: (String(constMainName.suffix(6)) + String(k_upValue)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.fontJump(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.auditoryImageTitle(name: (userFromText.replacingOccurrences(of: "make", with: "o") + "_talk" + String(showRowName.prefix(6)) + const_cellName.replacingOccurrences(of: "match", with: "s")))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(showImageColorData)))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(showEqualText.suffix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.latchkey(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(dataRangeContent) + String(noti_toName) + String(data_modeValue.suffix(5)) + "nts~").localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.auditoryImageTitle(name: (String(userTextAtViewTitle) + String(appBottomText)))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension AppMsgCell {
    ...
    override func deed(with data: ViewButtonReactiveCompatible) {
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (SceneAppManager.share.loginUid == String(userModel.uid)) ? SceneAppManager.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (SceneAppManager.share.loginUid == String(userModel.uid)) ? SceneAppManager.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .scalp() : .photoEvent()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.fontJump(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(WithSchemaConvertible.male.rawValue) ? String(bytes: app_calendarValue.map{$0^201}, encoding: .utf8)! : (String(notiSizeData.prefix(5)) + "fema" + String(showSystemResultName.suffix(6)) + "ault")
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.auditoryImageTitle(name: imgStr), for: .normal)
        ...
        self.avatarView.fileFinish(urlStr: userModel.headPic)
        ...
        self.iconBorder.countUser(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && SceneAppManager.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && SceneAppManager.share.appStatus == HighStatusValueConvertible.normal.rawValue)
        ...
        self.coinLabel.text = (dataErrorDayElseName.replacingOccurrences(of: "collection", with: "+") + String(user_partyData)).localizedPlace(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if WithChatManager.levelVoice(newData.direction,
        ...
        if WithChatManager.dataNext(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(userTextAtViewTitle) + String(appBottomText))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if WithChatManager.chemistryLab(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(constViewPriceStatusTitle.prefix(4)) + "read_pre")
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.auditoryImageTitle(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (SceneAppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? SceneAppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (SceneAppManager.share.loginUid == String(temCellData.msgModel.user.uid)) ? SceneAppManager.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
