...
/*: "mf/call/GetUserInfo" :*/
fileprivate let app_fromText:String = "self longmf/cal"
fileprivate let constViewData:String = "l/GetUsebegin size"
fileprivate let appCardName:String = "color pic caserInfo"

/*: "uid" :*/
fileprivate let k_succeedValue:[UInt8] = [0x64,0x69,0x75]

/*: "TRTC 随机视频匹配中." :*/
fileprivate let noti_requestValue:[Character] = ["T","R","T","C"]
fileprivate let showMakeData:String = " 随\u{673a}视频匹配中."

/*: "TRTC  :*/
fileprivate let constFrameDayValue:String = "TRTC let button pair field camera"

/*: "发起" :*/
fileprivate let app_clearData:[UInt8] = [0xec,0x86,0x98,0xe1,0xbc,0xbe]

private func pathImagePop(field num: UInt8) -> UInt8 {
    return num ^ 9
}

/*: "收到" :*/
fileprivate let k_dataValue:[Character] = ["收","\u{5230}"]

/*: 视频通话. logId: :*/
fileprivate let noti_valueTapLogTitle:String = "\u{89c6}频通话"

/*: ." :*/
fileprivate let notiSucceedActionValue:String = "commit"

/*: "s- :*/
fileprivate let mainRecordValue:[Character] = ["s","-"]

/*: - :*/
fileprivate let mainHungTitle:String = "-"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let constVideoValue:String = "dataTC"
fileprivate let user_frameName:[Character] = [" ","视","频","通","话","销","\u{6bc1}","."," ","l","o","g","I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let noti_objectTitle:String = "trtc"
fileprivate let show_colorStarText:[Character] = ["误","."," ","e","r","r","C","o","d","e",":"]

/*: , errMsg: :*/
fileprivate let mainUpData:String = "as show price, er"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let data_kitActualName:String = "origin true hiddenTRTC "
fileprivate let showEqualData:String = "SDK收\u{5230}"
fileprivate let user_rangeValue:String = "rningCif equal with"

/*: , warningMsg: :*/
fileprivate let notiPriceValue:String = "text text, w"
fileprivate let kToContent:String = "ARNI"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let user_modelHiddenData:[Character] = ["T","R","T","C"," ","视","频","通","话","进","入","房","间","."," "]
fileprivate let userMidData:String = "result:at birth actual dismiss let"

/*: , logId： :*/
fileprivate let kRemoveName:[Character] = [","," ","l","o","g","I","d"]
fileprivate let const_delayValue:[Character] = ["："]

/*:  加入了房间" :*/
fileprivate let mainInfoSucceedValue:String = " 加\u{5165}了房"
fileprivate let data_normalPlayerContent:[Character] = ["\u{95f4}"]

/*:  离开了房间" :*/
fileprivate let app_ofValue:[Character] = [" ","\u{79bb}","开","了","房","间"]
...
// __CLOSE_PRINT__
//
//  ShouldViewReactiveCompatible.swift
//  AbroadTalking
//
...
enum ViewDirectionChatSituation {
    //: case Start
    case Start // 发起通话（非通话）
    //: case Recieved
...
class ConfigSceneReactiveCompatible: NSObject {
    ...
    var remoteCameraStatus = CapturePlaceCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class ShouldViewReactiveCompatible: NSObject {
    ...
    deinit {
        ...
        self.laborResourcesEveryBringOutShould()
    }
}
...
extension ShouldViewReactiveCompatible {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func supplyLine(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = UpRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(app_fromText.suffix(6)) + String(constViewData.prefix(8)) + String(appCardName.suffix(5)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: k_succeedValue.reversed(), encoding: .utf8)!: uid]
...
extension ShouldViewReactiveCompatible {
    ...
    func laborResourcesEveryBringOutShould() {
        //: destoryTRTCView()
        statusPrompt()
    }
    ...
    func effectInCamera(_ frontCamera: Bool) {
        ...
        self.sinceNumberimate(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func addUp(_ openCamera: Bool) {
        ...
        self.vanguardEnable(isOpen: openCamera)
    }
    ...
    func beforeShow() {
        ...
        DispatchQueue.main.async {
            ...
            self.makeView(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension ShouldViewReactiveCompatible {
    ...
    private func sinceNumberimate(front: Bool) {
    ...
    private func vanguardEnable(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_errorFormat == true else {
            //: if isOpen {
            if isOpen {
...
extension ShouldViewReactiveCompatible {
    ...
    func toolEqual() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(useNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(mentalFaculty(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sumerrupted(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(audioNameGift(notif:)),
    ...
    @objc private func useNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func mentalFaculty(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func sumerrupted(notif: Notification) {
    ...
    @objc func audioNameGift(notif: Notification) {
...
extension ShouldViewReactiveCompatible {
    ...
    func equalSearch(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            NurseLogReactiveCompatible.citeOrLineup(msg: (String(noti_requestValue) + showMakeData))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            NurseLogReactiveCompatible.citeOrLineup(msg: (String(constFrameDayValue.prefix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (noti_valueTapLogTitle + ". logId:") + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            makeView(smallWindow: false)
    ...
    func makeView(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_errorFormat == true else {
    ...
    private func statusPrompt() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        NurseLogReactiveCompatible.citeOrLineup(msg: (constVideoValue.replacingOccurrences(of: "data", with: "TR") + String(user_frameName)) + "\(infoModel?.logId ?? 0).")
...
extension ShouldViewReactiveCompatible: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NurseLogReactiveCompatible.citeOrLineup(msg: (noti_objectTitle.uppercased() + " 视\u{9891}\u{901a}话\u{9519}" + String(show_colorStarText)) + "\(errCode)" + (String(mainUpData.suffix(4)) + "rMsg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NurseLogReactiveCompatible.citeOrLineup(msg: (String(data_kitActualName.suffix(5)) + "视频通\u{8bdd}" + showEqualData + "\u{8b66}告. wa" + String(user_rangeValue.prefix(6)) + "ode:") + "\(warningCode)" + (String(notiPriceValue.suffix(3)) + kToContent.lowercased() + "ngMsg:") + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: kTopPath, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        NurseLogReactiveCompatible.citeOrLineup(msg: (String(user_modelHiddenData) + String(userMidData.prefix(7))) + "\(result)" + (String(kRemoveName) + String(const_delayValue)) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (mainInfoSucceedValue + String(data_normalPlayerContent)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(app_ofValue)))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != SceneAppManager.share.loginUserMode.userID else { return }
        guard userId != SceneAppManager.share.loginUserMode.userID else { return }
