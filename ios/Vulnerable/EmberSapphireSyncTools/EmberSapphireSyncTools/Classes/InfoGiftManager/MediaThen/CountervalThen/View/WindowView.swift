...
fileprivate let appLabName:[UInt8] = [0x83,0x84,0x83,0x9e,0xc2,0x89,0x85,0x8e,0x8f,0x98,0xd0,0xc3,0xca,0x82,0x8b,0x99,0xca,0x84,0x85,0x9e,0xca,0x88,0x8f,0x8f,0x84,0xca,0x83,0x87,0x9a,0x86,0x8f,0x87,0x8f,0x84,0x9e,0x8f,0x8e]

private func phoneSystemLeading(player num: UInt8) -> UInt8 {
    return num ^ 234
}

/*: "#171717" :*/
fileprivate let appToValue:String = "#17171"
fileprivate let showBadShareTableValue:String = "make"

/*: "icon_cameraoff" :*/
fileprivate let userInfoTitle:String = "in user starticon_"
fileprivate let userViewData:[Character] = ["f","f"]
...
class WindowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.moveRgba(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appLabName.map{phoneSystemLeading(player: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (appToValue.capitalized + showBadShareTableValue.replacingOccurrences(of: "make", with: "7")))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.auditoryImageTitle(name: (String(userInfoTitle.suffix(5)) + "camerao" + String(userViewData)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension WindowView {
    ...
    func behindEqualCamera(urlStr: String?, isOpenCamera: CapturePlaceCameraStatus) {
...
extension WindowView: UIGestureRecognizerDelegate {
    ...
    @objc private func vocalization(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, main_clickBottomContent - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, data_senseEndMessage - self.frame.size.height * 0.5)
...
extension WindowView {
    ...
    private func lacrossePlayer() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(vid))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(vocalization(pan:)))
        //: pan.delegate = self
        pan.delegate = self
