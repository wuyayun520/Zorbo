...
/*: "id" :*/
fileprivate let data_pathDeviceName:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let notiAcrossText:[Character] = ["n","u","m"]
...
@objc public enum ScreenListSendable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ToTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ScreenListSendable
    ...
    mutating func makeInstance(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct WithAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(notiAcrossText))
    }
}
