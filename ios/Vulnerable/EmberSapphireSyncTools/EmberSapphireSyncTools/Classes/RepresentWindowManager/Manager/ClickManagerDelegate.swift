...
fileprivate let main_thenData:[UInt8] = [0xc9,0xca,0xc6,0xc4,0xd1,0xcc,0xca,0xcb,0x85,0xc4,0xd0,0xd1,0xcd,0xca,0xd7,0xcc,0xdf,0xc4,0xd1,0xcc,0xca,0xcb,0xf6,0xd1,0xc4,0xd1,0xd0,0xd6,0x85,0xcc,0xd6,0x85]

private func videoTemp(make num: UInt8) -> UInt8 {
    return num ^ 165
}

/*: "get location failed. error: :*/
fileprivate let noti_equalPageValue:String = "get lomodel suite size import"
fileprivate let noti_blackArrayTitle:String = "on fajump quote"
fileprivate let user_toData:String = " error:model height color self error"
...
enum EffectMediaRepresentable: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class ClickManagerDelegate: NSObject {
    ...
    var locationType: EffectMediaRepresentable?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: ClickManagerDelegate?
    //: class func shared() -> TalkingLocationManager {
    class func databaseImage() -> ClickManagerDelegate {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = ClickManagerDelegate()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func messageBar(block: @escaping Locationblock) {
    ...
    func fullPhaseOfTheMoon(block: @escaping Authblock) {
    ...
    func gesturePermission() -> EffectMediaRepresentable {
    ...
    func addDrop() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: main_thenData.map{videoTemp(make: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension ClickManagerDelegate: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(noti_equalPageValue.prefix(6)) + "cati" + String(noti_blackArrayTitle.prefix(5)) + "iled." + String(user_toData.prefix(7))) + "\(error.localizedDescription)")
    }
}
