...
class ContainerReactiveCompatible: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: StartChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = ConfigSceneReactiveCompatible()          // 视频渲染配置类
    ...
    private var trtcView: BeanReactiveCompatible?    // 预览view（使用商汤美颜）
...
extension ContainerReactiveCompatible {
    ...
    func releasePrepare() {
        //: destoryTRTCView()
        destoryInsidePosition()
    }
    
    /// 摄像头前后置切换
    //: func useFrontCamera(_ frontCamera: Bool) {
    func asTime(_ frontCamera: Bool) {
        ...
        self.alongGroup(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func afterLocationLab(_ openCamera: Bool) {
        ...
        self.timeRandom(isOpen: openCamera)
    }
    ...
    func firstNeed() {
        ...
        DispatchQueue.main.async {
            ...
            self.commitTableWindow(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension ContainerReactiveCompatible {
    ...
    private func alongGroup(front: Bool) {
        ...
        self.trtcView?.intervaleraction(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func timeRandom(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_errorFormat == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.juxtaposeApplication(isOpen: isOpen)
...
extension ContainerReactiveCompatible {
    ...
    func analysis() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(backgroundAppear(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(addTarget(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(tillPushEqual(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(perspectiveNotif(notif:)),
    ...
    @objc private func backgroundAppear(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func addTarget(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func tillPushEqual(notif: Notification) {
    ...
    @objc func perspectiveNotif(notif: Notification) {
...
extension ContainerReactiveCompatible {
    ...
    func restoreAcross() {
        ...
        if self.infoModel?.videoStage == .Waiting {
            ...
            commitTableWindow(smallWindow: false)
        ...
        //: } else if self.infoModel?.videoStage == .Calling {
            ...
            //: } else {
                ...
                commitTableWindow(smallWindow: true)
            }
        }
    ...
    func commitTableWindow(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_errorFormat == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = BeanReactiveCompatible()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.bagPusher(isSmallWindow: smallWindow)
        //: if smallWindow { 
...
//    }
    ...
    private func destoryInsidePosition() {
