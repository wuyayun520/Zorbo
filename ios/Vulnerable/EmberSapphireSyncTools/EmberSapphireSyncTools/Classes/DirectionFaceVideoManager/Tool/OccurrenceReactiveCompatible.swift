...
/*: "%@%@" :*/
fileprivate let notiWhiteData:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let k_dropViewData:String = "/TXCachematch disabled gallery model count"
...
// __CLOSE_PRINT__
//
//  OccurrenceReactiveCompatible.swift
//  AbroadTalking
//
...
protocol SceneButtonDelegateThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func applicationStatus(player: OccurrenceReactiveCompatible, status: StopPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func conversion(player: OccurrenceReactiveCompatible, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func imageTo(player: OccurrenceReactiveCompatible, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class OccurrenceReactiveCompatible: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: SceneButtonDelegateThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: StopPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = OccurrenceReactiveCompatible()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(k_dropViewData.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(stopMove), userInfo: nil, repeats: true)
...
extension OccurrenceReactiveCompatible {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func sound(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: userCreateName.caput(), with: constStatusStr)
    ...
    @objc func stopMove() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.imageTo(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func cuttingEdge(renderMode: InfoRenderMode) {
    ...
    func upwardly(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func outsideTotaleraction() -> Int {
    ...
    func toEnable(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isFinding == true {
        if StopThen.shared.isFinding == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func playerAdd() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.applicationStatus(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func addPlayings() -> Bool {
...
extension OccurrenceReactiveCompatible: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.conversion(player: self, duration: Int(duration), currentTime: Int(currentTime))
