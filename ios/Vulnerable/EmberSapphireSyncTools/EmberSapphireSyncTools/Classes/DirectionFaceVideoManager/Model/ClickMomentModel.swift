...
/*: "id" :*/
fileprivate let constManagerData:String = "ID"
...
struct ClickMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [TalkingBackStochasticProcessMeasurable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: TextMeasurable?
...
struct TalkingBackStochasticProcessMeasurable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct TextMeasurable: HandyJSON {
    ...
    var status: AllTableConvertible?
}

///
//: enum MomentVideoStatus: String {
enum AllTableConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ClickMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func currentPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(IndexThen.serration(text: content!, size: CGSize(width: main_clickBottomContent - 54, height: CGFloat(MAXFLOAT)), font: UIFont.handleFont(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == TurnScalarLiteral.Photo.rawValue || momentType == TurnScalarLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
