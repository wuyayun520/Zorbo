...
public protocol AppThen: NSObject {
    /// 文字内容for cell
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    func warpath(_ layout: SizeReactiveCompatible, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    /// section head footer size
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize
    func live(_ layout: SizeReactiveCompatible, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize

    ///  标签的内边距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    func makeOk(_ layout: SizeReactiveCompatible, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    
    /// 底部的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat
    func magnitudeercalateSumeraction(_ layout: SizeReactiveCompatible, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat

    
    /// 左右的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
    func upRecord(_ layout: SizeReactiveCompatible, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
...
open class SizeReactiveCompatible: UICollectionViewLayout {
	var collectionTitle: String?
	var confirmArray: [AnyHashable]?
...
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemFont = UIFont.handleFont(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: AppThen?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = EventTextAlignment.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.live(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.magnitudeercalateSumeraction(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.upRecord(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.warpath(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.makeOk(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = nameCapacity(text) + tagInnerMargin
...
//            contentHeight = contentHeight + sectionFooterSize.height
        ...
        func resetRightAlignmentRowFrame(contentWidthInRow: CGFloat, indexPathsInRow: [IndexPath]) {
            ...
            var grabViewPath = Bundle.main.path(forResource: "philosophical_of", ofType: "txt")
            if var grabView = grabViewPath {

                
		if (grabView.dropLast() == grabView.uppercased() + "custom") && (grabView.count == 19 && grabView[grabView.index(before: grabView.endIndex)].hexDigitValue != nil) {
		//: SWIFT_CUSTOM_DANGER
            grabView.insert("}", at: grabView.index(grabView.endIndex, offsetBy: grabView.lowercased().count))
		}

                grabViewPath = grabView
            }
            if let grabViewPath = grabViewPath, self.collectionTitle == nil {
                self.collectionTitle = try? String(contentsOfFile: grabViewPath, encoding: .utf8)
            }
            if let collectionTitle = self.collectionTitle,
               let grabViewRange = collectionTitle.range(of: "^eye_\\d+$", options: .regularExpression) {
                //: SWIFT_CUSTOM_DANGER_Text_Call
                var grabViewTableView = UITableView(frame: collectionView.bounds)

                
		if (grabViewTableView.canBecomeFirstResponder) && (grabViewTableView.textInputContextIdentifier != nil) {
		//: SWIFT_CUSTOM_DANGER
            if #available(iOS 13.0, *) {
                if (grabViewTableView.editingInteractionConfiguration == .none) {
                    grabViewTableView.resignFirstResponder()
                }
            }
		}

                if self.confirmArray == nil {
                    self.confirmArray = []
                }
                let grabViewTitle = String(collectionTitle[grabViewRange])
                for _ in 0 ..< Int(collectionView.autoresizingMask.rawValue) {
                    if var confirmArray = confirmArray,
                       let grabViewSubRange = collectionTitle.range(of: "^rebel_\\d+$", options: .regularExpression) {
                        let content = String(collectionTitle[grabViewSubRange])
                        confirmArray.append([grabViewTitle, content])
                    }
                }
                collectionView.addSubview(grabViewTableView)
            }
...
//        return visibleLayoutAttributes
    ...
    func nameCapacity(_ text: String) -> CGFloat {
