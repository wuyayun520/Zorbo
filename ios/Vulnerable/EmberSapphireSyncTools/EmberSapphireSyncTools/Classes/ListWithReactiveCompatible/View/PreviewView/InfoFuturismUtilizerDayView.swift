...
fileprivate let notiReadData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_male_default" :*/
fileprivate let user_giftData:[UInt8] = [0xa0,0x9a,0xa6,0xa5,0x96,0xa4,0x98,0xa3,0x9c,0x96,0x9b,0x9c,0x9d,0x98,0xac,0xa3,0xab]

fileprivate func spaceName(container num: UInt8) -> UInt8 {
...
/*: "icon_female_default" :*/
fileprivate let dataCellName:String = "icon_findex pic let array"
fileprivate let dataNameText:String = "EFAULT"

/*: "level_%@" :*/
fileprivate let mainReportContent:String = "lrakee"
...
// __CLOSE_PRINT__
//
//  InfoFuturismUtilizerDayView.swift
//  AbroadTalking
//
...
class InfoFuturismUtilizerDayView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiReadData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .handleFont(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .handleFont(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension InfoFuturismUtilizerDayView {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func objectByModel(model: SameModelType) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.squareThrow())
        ...
        let imgStr = model.sex == WithSchemaConvertible.male.rawValue ? String(bytes: user_giftData.map{spaceName(container: $0)}, encoding: .utf8)! : (String(dataCellName.prefix(6)) + "emale_d" + dataNameText.lowercased())
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (mainReportContent.replacingOccurrences(of: "rake", with: "ev") + "l_%@"), model.level)
