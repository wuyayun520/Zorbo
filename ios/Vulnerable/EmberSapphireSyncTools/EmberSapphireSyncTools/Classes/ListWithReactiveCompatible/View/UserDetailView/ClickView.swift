...
fileprivate let constEffectData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bth_me_chat_nor" :*/
fileprivate let notiColorText:String = "bth_show view"
fileprivate let appTitleContent:String = "country male share aspectt_no"
fileprivate let user_clickTitle:String = "R"

/*: "#6ADEFF" :*/
fileprivate let dataButtonText:[Character] = ["#","6","A","D","E","F","F"]

/*: "#5D6FFF" :*/
fileprivate let notiGiftTitle:[Character] = ["#"]
fileprivate let dataColorTitle:String = "5D6FFFbottom equal stage normal self"

/*: "icon_lounge" :*/
fileprivate let dataEnterText:String = "isizen"

/*: "icon_video_call" :*/
fileprivate let k_fillData:[Character] = ["i","c","o","n"]
fileprivate let mainEqualValue:[Character] = ["_","v","i","d","e","o","_","c","a","l","l"]

/*: "iv_me_vip" :*/
fileprivate let dataViewValue:[Character] = ["i","v","_","m","e","_","v","i","p"]

/*: "Edit" :*/
fileprivate let mainRequestName:String = "Editreturn let main"

/*: "#AB57F6" :*/
fileprivate let showCountName:String = "party path start#AB57F6"

/*: "#FC57B7" :*/
fileprivate let notiPlusName:[Character] = ["#"]
fileprivate let kPicTitle:String = "FC57value"

/*: "#FE9074" :*/
fileprivate let appIndicatorText:[Character] = ["#","F","E","9","0","7","4"]

/*: "Video Call" :*/
fileprivate let kTitleResultName:[Character] = ["V","i","d","e","o"]
fileprivate let data_mediumMakeValue:String = "right remove frame Call"
...
let appErrorErrMsg = (15 + actualWidth(w: 36) + 15 + constStatusContent)
...
class ClickView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constEffectData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var shadowLayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: main_clickBottomContent, height: 15 + actualWidth(w: 38) + 15)
    ...
    private lazy var chatBtn: UIImageView = {
        ...
        img.image = UIImage.auditoryImageTitle(name: (String(notiColorText.prefix(4)) + "me_cha" + String(appTitleContent.suffix(4)) + user_clickTitle.lowercased()))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(chatButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(pushBeauty))
    ...
    private lazy var videoBtn: UIButton = {
        ...
        let colors = [UIColor(hex: (String(dataButtonText)))!.cgColor, UIColor(hex: (String(notiGiftTitle) + String(dataColorTitle.prefix(6))))!.cgColor]
        ...
        btn.setBackgroundImage(UIImage.colored(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
        //: btn.layer.cornerRadius = actualWidth(w: 18)
        btn.layer.cornerRadius = actualWidth(w: 18)
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(videoButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(sizeOpen), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.auditoryImageTitle(name: (dataEnterText.replacingOccurrences(of: "size", with: "co") + "_lounge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var callIcon: UIImageView = {
        ...
        img.image = UIImage.auditoryImageTitle(name: (String(k_fillData) + String(mainEqualValue)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var maleVipPriceBtn: UIImageView = {
        ...
        img.image = UIImage.auditoryImageTitle(name: (String(dataViewValue)))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(maleVipPriceButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(incrementalCost))
        //: img.addGestureRecognizer(tap)
        img.addGestureRecognizer(tap)
        //: img.isHidden = true
        img.isHidden = true
    ...
    private lazy var videoCallManager: EventRequestObjectProtocol = .init()
    //: lazy var editBtn: UIButton = {
    lazy var editBtn: UIButton = {
        //: let btn = UIButton.init(type: .custom)
        let btn = UIButton(type: .custom)
        //: btn.setTitle("Edit".localized, for: .normal)
        btn.setTitle((String(mainRequestName.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(observerCurrent), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension ClickView {
    ...
    @objc private func incrementalCost() {
    ...
    @objc func sizeOpen() {
        ...
        noti_statusInviteScreenMessage.makeUp(eventID: kBottomFormat, toUid: userInfoModel.uid)
        //: if SceneAppManager.share.loginUserMode.sex == Gender.male.rawValue {
        if SceneAppManager.share.loginUserMode.sex == WithSchemaConvertible.male.rawValue { // 男性用户
            //: videoCallManager.checkAndCallVideo()
            videoCallManager.tableComplection()
            ...
            videoCallManager.duringCell(videoPrice: userInfoModel.videoBean, voicePrice: userInfoModel.voiceBean, vipPrompt: userInfoModel.prompt)
        }
    }

    /// chat
    //: @objc private func chatButtonClick() {
    @objc private func pushBeauty() {
        //: AccumulationThen.share.func__pushToPriveteChatVC(chatID: userInfoModel.uid)
        AccumulationThen.share.inputFor(chatID: userInfoModel.uid)
        //: uploadRecord.uploadRecordEvent(eventID: ClickProfileChatButton, toUid: userInfoModel.uid)
        noti_statusInviteScreenMessage.makeUp(eventID: const_screenStr, toUid: userInfoModel.uid)
    }

    /// edit
    //: @objc private func editButtonClick() {
    @objc private func observerCurrent() {
...
extension ClickView {
    ...
    func toView(_ userModel: AppSceneInfoModel) {
        ...
        // 根据角色 展示/隐藏 按钮
        //: if userModel.uid == SceneAppManager.share.loginUid {
        if userModel.uid == SceneAppManager.share.loginUid { // 本人资料页
            //: editBtn.isHidden = false
            editBtn.isHidden = false
            ...
            return
                    ...
                    //: } else if SceneAppManager.share.loginUid != TopReactiveCompatible.getCustomerServiceID(),
        } else if SceneAppManager.share.loginUid != TopReactiveCompatible.tied(),
                  //: SceneAppManager.share.loginUserMode.sex == userModel.sex,
                  SceneAppManager.share.loginUserMode.sex == userModel.sex,
        ...
        if SceneAppManager.share.loginUserMode.sex == WithSchemaConvertible.male.rawValue {
            ...
            if SceneAppManager.share.loginUserMode.loungePlus == false { // 男性非会员
                ...
                if SceneAppManager.share.appStatus == HighStatusValueConvertible.normal.rawValue {
                    ...
                    maleVipPriceLab.text = String.jumpVideo(videoPrice: userInfoModel.videoVIPPrice)
                }
                //: vipIcon.isHidden = true
                vipIcon.isHidden = true
                //: callStrLab.text = String.getMaleVIPVideoPriceString(videoPrice: userInfoModel.videoPrice)
                callStrLab.text = String.jumpVideo(videoPrice: userInfoModel.videoPrice)

                //: } else {
            } else {
                //: let colors = [UIColor(hex: "#AB57F6")!.cgColor, UIColor(hex: "#FC57B7")!.cgColor, UIColor(hex: "#FE9074")!.cgColor]
                let colors = [UIColor(hex: (String(showCountName.suffix(7))))!.cgColor, UIColor(hex: (String(notiPlusName) + kPicTitle.replacingOccurrences(of: "value", with: "B7")))!.cgColor, UIColor(hex: (String(appIndicatorText)))!.cgColor]
                ...
                videoBtn.setBackgroundImage(UIImage.colored(colors: colors, size: CGSize(width: actualWidth(w: 250), height: actualWidth(w: 36))), for: .normal)
                ...
                callStrLab.text = String.jumpVideo(videoPrice: userInfoModel.videoVIPPrice)
            }
            ...
            callStrLab.text = (String(kTitleResultName) + String(data_mediumMakeValue.suffix(5))).localized
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func cardPlayerForSubviews() {
    ...
    private func equalFromConstraint() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(constStatusContent)
        }
    }
}
