...
/*: / :*/
fileprivate let mainErrorName:[Character] = ["/"]

/*: "\n" :*/
fileprivate let user_currentModelText:String = "\n"
...
struct MissiveScreenObjectContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class ViewBarThen: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        stack(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        stack(placeholder: nil, placeholderColor: nil)
    }
}
...
extension ViewBarThen {
    ...
    private func stack(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(RegionLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(RegionLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-RegionLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: MissiveScreenObjectContainerInset().top, left: MissiveScreenObjectContainerInset().left, bottom: MissiveScreenObjectContainerInset().bottom, right: MissiveScreenObjectContainerInset().right)
...
extension ViewBarThen: UITextViewDelegate {
